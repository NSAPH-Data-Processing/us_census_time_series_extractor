...
rule all:
    input:
        expand("data/output/census_series/acs1_{geo_type}.csv", geo_type=["state", "county"]),
        expand("data/output/census_series/acs5_{geo_type}.csv", geo_type=["state", "county", "zcta"]),
        expand("data/output/census_series/sf1_{geo_type}.csv", geo_type=["state", "county"])


rule census_fetch_acs1:
    output:
        "data/output/census_series/acs1_{geo_type}.csv"
    params:
...
rule census_fetch_acs5:
    output:
        "data/output/census_series/acs5_{geo_type}.csv"
    params:
...
rule census_fetch_sf1:
    output:
        "data/output/census_series/sf1_{geo_type}.csv"
    params:
